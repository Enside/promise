...
class Promise {
  ...
  then(onFulfilled?: (value?: any) => any, onRejected?: (reason?: any) => any) {
    ...
    const task = this.createTask(deferredPromise, onFulfilled, onRejected)
    if (this.state === Status.Pending) {
  ...
  private Resolve(deferredPromise: deferredPromise, x: any) {
    //promise
    if (x instanceof Promise) {
      if (x === deferredPromise.promise) {
        deferredPromise.reject(new TypeError())
      } else {
        x.then(
          (y) => {
            this.Resolve(deferredPromise, y)
          },
          (r) => {
            deferredPromise.reject(r)
          }
        )
      }
      return
